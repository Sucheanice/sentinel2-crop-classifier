# -*- coding: utf-8 -*-
"""处理迅雷下载的 JP2 → 生成遂宁 summer 多波段 GeoTIFF"""
...
SRC_DIR = Path(r"E:\迅雷下载\08121721")
...
# 与 rebuild_suining_cube.py 一致的bbox
...
def main():
    t0 = time.time()
    log("=" * 60)
    log("处理遂宁 48RWU 夏季 S2 JP2 → 多波段 GeoTIFF")
    log("=" * 60)

    # 检查源文件
    for b in S2_BANDS_ALL:
        if not (SRC_DIR / f"{b}.jp2").exists():
            log(f"[FATAL] {b}.jp2 不存在")
            return
